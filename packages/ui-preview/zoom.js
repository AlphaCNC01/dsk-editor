// ============================================================================
// zoom.js — manual zoom/pan for a live SVG preview pane. Purely a
// screen-space CSS transform on <svg id="stage"> itself (translate+scale
// in CSS pixels), layered on top of whatever fit-to-content viewBox logic
// the calling app's own render loop already does — it never touches the
// viewBox, geometry, or any export path, so this is strictly a preview
// convenience.
//
// Shared between apps/engineer and apps/geometry-editor (moved here,
// into packages/ui-preview, rather than duplicated per app, once the
// ES-module conversion made "just import the same file" straightforward
// — previously this was a deliberate copy-paste, with a comment in the
// geometry-editor's own copy saying to re-copy by hand if the original
// ever changed). Both apps' own index.html markup uses the same
// #preview/#stage/#zoomOutBtn/#zoomInBtn/#zoomResetBtn ids, which is what
// makes sharing this file directly possible with zero per-app branching.
//
...
//
// initZoom() runs the DOM-querying/listener-wiring at CALL time, not at
// module-load time — this file is meant to be dynamically imported (or
// statically imported and then explicitly called) only once the caller's
// own DOM already exists, the same guarantee the old <script> tag's
// position at the end of <body> used to provide implicitly. See either
// app's own main.js for the actual call site.
// ============================================================================
