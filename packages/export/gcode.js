...
import * as ClipperLibImport from 'clipper-lib';

// clipper-lib is an older CommonJS package (module.exports = { ... }, not a
// native ES module) — Vite/Node's ESM interop exposes that whole exports
// object as the default export on some builds and spreads it across named
// exports on others depending on how the CJS module is shaped, so this
// checks both rather than assuming one. Resolved once at module-load time
// (not inside requireClipperLib below) since imports are static: unlike
// the old CDN-<script>-tag version, there's no "did the network request
// finish yet" race to guard against any more — either the import
// succeeded when this module loaded, or the whole app failed to build.
const ClipperLib = ClipperLibImport.default && ClipperLibImport.default.ClipperOffset
  ? ClipperLibImport.default
  : ClipperLibImport;
...
export const ExportGCode = (() => {
  ...
  const TOOLS = {
    '4x17': {
      ...
      rpm: 24000, feed: 3000, plungeFeed: 1200,
      depthPerPass: 6,   // mm of Z per pass — small bit, shallow bites
      stepover: 1.6,     // mm between concentric pocket-clearing rings
    },
    '4x22': {
    ...
    '8x32': {
      ...
      rpm: 18000, feed: 4000, plungeFeed: 1500,
      depthPerPass: 9.25, // matches the shop's own reference file's own step
      stepover: 3.2,
    },
    '8x42': {
  ...
  // based CAM/laser-cutting tools), installed as the clipper-lib npm
  // dependency and imported at the top of this file -- see clipperOffset
  ...
  function requireClipperLib(){
    if (!ClipperLib || !ClipperLib.ClipperOffset) {
      throw new Error(
        'ClipperLib failed to load correctly from the clipper-lib package. ' +
        'This points at a packaging/interop problem (see this file\'s own ' +
        'import comment for the CJS/ESM interop this resolves), not a missing ' +
        'network request — check that clipper-lib is listed in package.json ' +
        'and installed.'
      );
    }
    return ClipperLib;
