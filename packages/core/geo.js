...
export const Geo = (() => {
  ...
  // Decimal.js is now a real npm dependency (see package.json), imported
  // directly above, rather than an optional CDN <script> tag — so unlike
  // the pre-monorepo version of this file, there's no "did the CDN load"
  // fallback path to worry about here any more; if the import fails the
  // whole build fails loudly, which is the outcome we want.
