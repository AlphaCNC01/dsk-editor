...
//   or directly: node test/resize.test.js
//
// resize.js is not itself an ES module (no import/export — it's a plain
// self-executing script guarded by its own DOMContentLoaded check, same
// as before the ES-module conversion), so it needs no import-time
// change at all: this test just sets up the DOM stub as a global, then
// dynamically imports the file so its own top-level code runs against
// that stub, same as the old vm.runInContext did.
// ============================================================================
...
global.getComputedStyle = (el) => ({ getPropertyValue: (k) => el.style.getPropertyValue(k) });

await import('../js/resize.js');
...
if (currentWidthPx() !== 480) fail('expected 480');
...
if (currentWidthPx() !== 260) fail('expected clamped 260');
...
if (currentWidthPx() !== 720) fail('expected clamped 720');
...
if (currentWidthPx() !== widthAfterStop) fail('drag continued after mouseup');

// Double-click resets to default
handle.dispatchEvent('dblclick', {});
console.log('width after double-click reset (should be 380):', currentWidthPx());
if (currentWidthPx() !== 380) fail('expected reset to 380');
