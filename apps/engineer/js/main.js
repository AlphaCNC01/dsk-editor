...
// #zoomControls) at module-load time, so it's imported dynamically here,
// inside bootWithZoom(), rather than as a static top-level import — that
// guarantees it only runs once the DOM it needs actually exists (see this
// file's own DOMContentLoaded guard below), the same guarantee the old
// <script> tag's position at the end of <body> used to provide implicitly.
async function bootWithZoom(){
  const { resetZoom } = await import('./ui/zoom.js');
  boot(resetZoom);
}
