// ============================================================================
// UI field helpers + createEmbeddableManager — shared markup builders
...
// createEmbeddableManager, which auto-creates its own UI section, wires
// up its instance list (via createInstanceListManager from
// instance-list.js), and resolves per-instance defaults.
// ============================================================================

import { Elements } from '@core/elements.js';
import { el, createInstanceListManager } from './instance-list.js';
...
// 3x3 grid layout order (row-major, matches how the buttons visually
  ...
  export function anchorGridHtml(idx, currentAnchor){
  ...
  export function transformFieldsHtml(idx, inst){
    return `
      <div class="row2">
        <div class="field">
  ...
  export function cableTargetSelectHtml(idx, inst, label){
    return `
      <div class="field">
        <label>${label}</label>
  ...
  export function updateCableTargetOptions(containerId, items, p){
  ...
  export function insetFieldsHtml(idx, inst) {
  ...
  export function ensureElementSection(key, label, addLabel, insertBeforeId){
  ...
  // onChange/onFieldChange are supplied by the CALLER (element-managers.js)
  // rather than this file reaching for a shared render()/debouncedRender()
  // global itself — field-helpers.js sits BELOW app-render.js in the
  // dependency graph (app-render.js's own readParams/render loop over
  // every manager this file builds), so importing render() directly here
  // would be a real circular import between the two modules. Passing the
  // callback in keeps the dependency one-directional.
  export function createEmbeddableManager(cfg) {
    const { key, label, typeSelector = null, cableTargetLabel = null, cableTargetWhen = null, addLabel, dimensionsDefault = true, typeDefaults = null, onChange, onFieldChange } = cfg;
