...
// Reuses buildProjectData/applyProjectData (from app-project.js's own
// initProjectIO), so a stored/shared snapshot is exactly the same shape
// as a saved .json project file. This is the LAST thing main.js calls,
// and is the one that fires the very first render(), choosing the source
// in priority order: URL link (if the page was opened via a shared link)
// > this tab's own session draft > blank form.
//
...
// across tabs/restarts) lives separately in app-history.js (see
// initHistory). This file's draft also carries the current
// historySessionId (via getHistorySessionId/setHistorySessionId, passed
// in below) so a reload of the same tab resumes writing to the same
// history entry instead of forking a new one.
//
// Exported as initAutosave(deps) — called LAST from main.js, once every
// other piece (initRender, initProjectIO, initHistory) already exists —
// same orchestration pattern as the rest of js/ui (see initProjectIO's
// own comment).
// ============================================================================

export function initAutosave({
  el, inputs, buildProjectData, applyProjectData, render,
  startNewHistorySession, upsertHistoryEntry,
  getHistorySessionId, setHistorySessionId,
}){
  ...
  function saveDraftLocally(){
    try {
      ...
      sessionStorage.setItem(STORAGE_KEY, JSON.stringify({ data, historySessionId: getHistorySessionId() }));
    } catch (err) {
  ...
  async function loadInitialState(){
    ...
    if (draft) {
      try {
        ...
        if (draft.historySessionId) setHistorySessionId(draft.historySessionId);
        return;
      } catch (err) {
  ...
  // Every input already triggers render()/debouncedRender() elsewhere
