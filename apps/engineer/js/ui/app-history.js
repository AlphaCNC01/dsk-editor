...
// (keyed by the module-local historySessionId below), rather than flooding the list with near-
...
// initProjectIO), so a history snapshot is exactly the same shape as a
// saved .json file or the sessionStorage draft.
//
// Exported as initHistory(deps) — an orchestration entry point called
// once from this app's own main.js, same pattern as initProjectIO (see
// its own comment). historySessionId itself is kept as a module-local
// `let` (not a returned value main.js re-stores anywhere) since only
// this file and app-state.js's own sessionStorage draft need to read or
// change it — getHistorySessionId/setHistorySessionId below are the
// only way in or out, so main.js just wires those two through to
// app-state.js's own initAutosave call.
// ============================================================================

let historySessionId = null;

export function getHistorySessionId(){ return historySessionId; }
export function setHistorySessionId(id){ historySessionId = id; }

export function initHistory({ el, inputs, buildProjectData, applyProjectData }){
  ...
  historySessionId = makeSessionId();
  ...
  function upsertHistoryEntry(){
    ...
    list = list.filter(entry => entry.id !== historySessionId);
    list.unshift({
      id: historySessionId,
      orderNumber,
      updatedAt: Date.now(),
  ...
  return { startNewHistorySession, upsertHistoryEntry, renderHistoryMenu };
}
