...
// preview), and the DXF/SVG/PNG/G-code export actions. See app-project.js
// for save/load and the final event-wiring + first render() call.
//
// Exported as initRender(deps) — an orchestration entry point called once
// from this app's own main.js, after buildElementManagers() (needs its
// returned array — see readParams/render below) — same pattern as
// initProjectIO/initHistory (see their own comments). Returns everything
// those two need in turn: inputs, sanitizeFilename, readParams, render,
// debouncedRender, runExport, and the export-menu toggles.
// ============================================================================

export function initRender({ el, elementManagers, Drawing, Dimensions, RenderSVG, ExportSVG, ExportDXF, ExportPNG, ExportGCode }){
  ...
  function readParams(){
    ...
    for (const { key, mgr } of elementManagers) p[key] = mgr.resolveItems(p);
  ...
  function render(){
    ...
    for (const { mgr } of elementManagers) mgr.updatePlaceholders(p);
  ...
  function exportGcode(){
    const p = readParams();
    if (p.W <= 0 || p.H <= 0) { alert('Укажите положительные ширину и высоту.'); return; }
    // Dimension lines carry no `machining` in layers.js, so ExportGCode
    // already skips them on its own; passed as just Drawing's own
    // entries here (no need to add dims.entries) for the same result.
    const entries = Drawing.build(p);
    ExportGCode.downloadAs(entries, p, (inputs.orderNumber.value || '').trim());
  }
