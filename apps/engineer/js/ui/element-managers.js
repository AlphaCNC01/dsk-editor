// ============================================================================
// UI element managers — buildElementManagers() below makes one
// createEmbeddableManager call per repeatable element, and returns the
// registry every other generic loop (readParams, render, buildProjectData,
// applyProjectData) iterates over instead of hand-listing every element
// separately. See the "Adding a new element" guide referenced below for
// the full walkthrough.
// ============================================================================

import { createInstanceListManager, el } from './instance-list.js';
import { ensureElementSection, createEmbeddableManager } from './field-helpers.js';
import { PULT_VARIANTS } from '@elements/pult.js';
import { CABLE_POCKET_VARIANTS } from '@elements/cable-pocket.js';
...
// Builds typeSelector.options straight from an element's own VARIANTS
  ...
  // (createEmbeddableManager, see field-helpers.js). insetFieldsHtml
  ...
  // manager below.
  //
  // Built as an exported FACTORY function (called once from this app's
  // own main.js, after app-render.js's own render/debouncedRender exist)
  // rather than a bare module-level side effect: every manager below
  // needs to trigger a re-render on change, but importing render()
  // directly here would create a real circular import with app-render.js
  // (which itself needs this file's own returned ELEMENT_MANAGERS array
  // — see readParams/render there). Taking render/debouncedRender as
  // parameters instead keeps the dependency one-directional, the same
  // fix field-helpers.js's own createEmbeddableManager uses (see its own
  // comment).
export function buildElementManagers({ render, debouncedRender }){
  ...
  // createEmbeddableManager) — the panel's own visual order comes
  ...
  // createInstanceListManager (the same base createEmbeddableManager
  ...
  const topNotchMgr = (() => {
    const { containerId, addBtnId } = ensureElementSection('topNotches', 'Вырезы для проводов', 'Добавить вырез');
  ...
  const pultMgr = createEmbeddableManager({
    onChange: () => render(), onFieldChange: () => debouncedRender(),
