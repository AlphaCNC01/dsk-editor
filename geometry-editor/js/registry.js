...
// shape — an arbitrary number of contour entries, each with its own
...
//   { sku, label, entries: [ { layer, verts: [{x,y,bulge}...] }
//     | { hole: { x, y, kind } } ] }
// `entries` is deliberately a 1:1 mirror of the real `parts` array — no
// grouping, no editor-only name field — so the text pane (see EditorState)
// can show literally what belongs inside `parts: [ ... ]`, and Save can
// write it back out with no reshaping beyond formatting. This is the same
// principle contour-list.js used to add on top (friendlier names, grouped
// multi-contour parts) — removed because it produced a second, DIFFERENT
// representation of the same data that didn't paste back into an element
// file as-is; this format is the one that does.
//
...
// the time Elements.all() exposes it. entriesFromParts() reconstructs that
...
// collapsed back into a single { hole: {x,y,kind} } entry, with (x,y) read
...
const Registry = (() => {
  ...
  // parts: [{verts, layer}] -> entries: one entry per array item EXCEPT
  // recognized hole/counterbore pairs, which collapse to one { hole }
  // entry each. A pair is recognized when two ADJACENT parts are exactly
    ...
    while (i < parts.length) {
      ...
      if (info && info.role === 'hole' && nextInfo && nextInfo.role === 'cs' && nextInfo.kind === info.kind) {
        ...
        if (c1 && c2 && sameCenter && radiiMatch) {
          entries.push({ hole: { x: c1.x, y: c1.y, kind: info.kind } });
          i += 2;
          continue;
        }
      }
      entries.push({ layer: part.layer || 'PHYSICAL', verts: part.verts });
      i++;
    }
    return entries;
  }
  ...
  function normalizeVariant(element, key){
    ...
    const entries = entriesFromParts(variant.parts);
    return {
  ...
  // Deep-clones a normalized variant's entries so editing in this app can
  ...
  function cloneEntries(entries){
    return entries.map(e => e.hole
      ? { hole: { ...e.hole } }
      : { layer: e.layer, verts: e.verts.map(v => ({ x: v.x, y: v.y, bulge: v.bulge || 0 })) });
  }

  return { editableElements, elementKind, normalizeVariant, cloneEntries };
})();
